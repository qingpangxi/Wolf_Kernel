...
//--SSCR-- for touch and power key
void SimulateKey(int keycode,int DownFlag){					
	struct input_dev *dev = g_input_dev;
	int val = 0;
	int row = 0, col = 0;
	
	//printk("SimulateKey  : %d %d...\n",keycode,DownFlag);	
	...
	val = MATRIX_SCAN_CODE(row, col, g_row_shift);
	
	input_event(dev, EV_MSC, MSC_SCAN, val);
	...
	if(DownFlag == 1){				
		...
		printk("Please Check the  Key...\n");	

	input_sync(dev);	
	}
	
}
EXPORT_SYMBOL_GPL(SimulateKey);
...
static int samsung_keypad_resume(struct device *dev)
{
	 ...
	 */
	 mask = readl(S5P_WAKEUP_STAT);
        printk("Keypad: WAKEUP_STAT=0x%x\n", mask);
        /* After suspend, report POWER_KEY in case of resuming by only KEY and EINT */
      
	//printk("*************samsung_keypad_resume: 0x%x --0x%x -- 0x%x--  0x%x***********************\n",
	//eint_pending1,eint_pending2,	eint_pending3,eint_pending4);
	//if ((eint_pending2 & (1<<5) )||(eint_pending3 &(1<<6))||(eint_pending4& (1<<2))) {
	if(mask & 0x21) {
		SimulateKey(KEY_POWER,1);
		mdelay(0);
		SimulateKey(KEY_POWER,0);
